...
static http_request_t* http_request_create(const char* url, const char* method, http_request_on_event_t on_event, void* ctx, 
...
http_request_t* http_request_create_put(const char* url, http_request_on_event_t on_event, void* ctx, 
    const char* content_type, const void* data, uint32_t data_size) {
  return_value_if_fail(url != NULL && on_event != NULL && content_type != NULL && data != NULL, NULL);
